...
import openpyxl
import argparse
from image_ml_utils.plots import colors, plot_one_box_kpt
from camera import UrlCamera
from poseestimator import PoseEstimator
from poseclassifier import SymbolicPoseClassifier
    ...
    def __init__(self, opt_arg):
        ...
        if source != 0:
            ...
            for i, url in enumerate(self.urls):
                urlcam = UrlCamera(url, rot_angle=rot_angles[i], source_is_auto_refresh=(IPCAMS_DATABASE_STARTCOL == 4))
                urlcam.start()
                self.cameras.append(urlcam)

        else:
            n = 1
            for i in range(n):
                urlcam = UrlCamera(0, rot_angle=0)
                urlcam.start()
                self.cameras.append(urlcam)
