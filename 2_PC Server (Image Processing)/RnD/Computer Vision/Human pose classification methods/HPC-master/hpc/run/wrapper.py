...
from z.core.pose_estimation import PoseEstimation
...
class Wrapper:
    ...
    def __getModel(self, model, gpuMode):
        ...
        if type(model) is str or (type(model) is tuple and len(model) == 1):  # one model
            modName = model
            dynName = None
        else:  # two yolo_models_utils
            modName = model[0]
            dynName = model[1]

        self.model = tf.keras.models.load_model('dataF/yolo_models_utils/' + modName)
        print("Model " + modName + " loaded.")
        print(self.model.summary())
        if dynName is not None:
            self.dynModel = tf.keras.models.load_model('dataF/yolo_models_utils/' + dynName)
